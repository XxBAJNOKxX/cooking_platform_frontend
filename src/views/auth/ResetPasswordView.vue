...
<template>
  <AuthLayout>
    ...
    <form v-else @submit.prevent="handleSubmit" class="flex flex-col gap-5">
      ...
      <BaseInput v-model="email" type="email" label="E-mail cím" id="rp-email"
        :error="errors.email?.[0]" :disabled="true" />

      <BaseInput v-model="password" type="password" label="Új jelszó" placeholder="Legalább 8 karakter"
        id="rp-password" :error="errors.password?.[0]" required :disabled="loading" />

      <BaseInput v-model="passwordConfirmation" type="password" label="Jelszó megerősítése" placeholder="••••••••"
        id="rp-password-confirmation" :error="errors.password_confirmation?.[0]" required :disabled="loading" />
