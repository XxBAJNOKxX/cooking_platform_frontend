...
const activeItemLabel = computed(
  () => TABS.find((t) => t.key === activeTab.value)?.item ?? '',
)
...
function endpointFor(tab, page = 1) {
  ...
  switch (tab) {
    case 'users':       return ['/admin/users', params]
    case 'recipes':     return ['/recipes', params]
    case 'tools':       return ['/kitchen-tools', params]
    case 'reviews':     return ['/reviews', params]
    case 'categories':  return ['/categories', { ...params, paginate: 1 }]
    case 'ingredients': return ['/ingredients', params]
    case 'units':       return ['/units', { ...params, paginate: 1 }]
  }
}
...
function startEdit(row) {
  ...
  if (activeTab.value === 'users') {
    editing.value = {
      entity: 'users',
      id: row.id,
      form: { username: row.username ?? '', email: row.email ?? '', bio: row.bio ?? '', is_admin: !!row.is_admin },
    }
...
<template>
  <div class="admin-root">
    ...
    <div class="admin-tools" v-if="['users', 'recipes', 'tools', 'reviews', 'ingredients', 'categories', 'units'].includes(activeTab)">
      <input
        v-model="search"
        type="text"
        class="admin-search"
    ...
    <div v-else class="admin-table-wrap">
      <table class="admin-table">
        ...
        <thead v-if="activeTab === 'users'">
          <tr><th>ID</th><th>Név</th><th>Email</th><th>Receptek</th><th>Eszközök</th><th>Admin</th><th></th></tr>
        </thead>
        <tbody v-if="activeTab === 'users'">
          <tr v-for="u in rows" :key="u.id">
            ...
            <td class="admin-actions">
              ...
              <button class="admin-del" :disabled="u.id === authStore.user?.id" @click="confirmDelete(u)">Törlés</button>
            </td>
          </tr>
        </tbody>
        ...
        <thead v-if="activeTab === 'recipes'">
          <tr><th>ID</th><th>Cím</th><th>Szerző</th><th>Nehézség</th><th></th></tr>
        </thead>
        <tbody v-if="activeTab === 'recipes'">
          <tr v-for="r in rows" :key="r.id">
            ...
            <td class="admin-muted">
              <RouterLink v-if="r.author?.id" :to="{ name: 'user-profile', params: { id: r.author.id } }" class="admin-link">
                {{ r.author.username }}
              </RouterLink>
              <span v-else>—</span>
            </td>
            <td>{{ r.difficulty }}</td>
            <td class="admin-actions">
              <RouterLink :to="{ name: 'recipe-edit', params: { id: r.id } }" class="admin-edit">Szerk.</RouterLink>
        ...
        <thead v-if="activeTab === 'tools'">
          <tr><th>ID</th><th>Név</th><th>Tulajdonos</th><th>Ár/nap</th><th>Elérhető</th><th></th></tr>
        </thead>
        <tbody v-if="activeTab === 'tools'">
          <tr v-for="t in rows" :key="t.id">
            ...
            <td class="admin-muted">
              <RouterLink v-if="t.owner?.id" :to="{ name: 'user-profile', params: { id: t.owner.id } }" class="admin-link">
                {{ t.owner.username }}
              </RouterLink>
              <span v-else>—</span>
            </td>
            ...
            <td class="admin-actions">
              <RouterLink :to="{ name: 'tool-edit', params: { id: t.id } }" class="admin-edit">Szerk.</RouterLink>
        ...
        <thead v-if="activeTab === 'reviews'">
          <tr><th>ID</th><th>Csillagok</th><th>Hozzászólás</th><th>Szerző</th><th>Recept</th><th></th></tr>
        </thead>
        <tbody v-if="activeTab === 'reviews'">
          <tr v-for="r in rows" :key="r.id">
            ...
            <td>
              <RouterLink v-if="r.user?.id" :to="{ name: 'user-profile', params: { id: r.user.id } }" class="admin-link">
                {{ r.user.username }}
              </RouterLink>
              <span v-else>—</span>
            </td>
            <td>
              <RouterLink :to="{ name: 'recipe-detail', params: { id: r.recipe?.id ?? r.recipe_id } }" class="admin-link">
                {{ r.recipe?.title ?? `#${r.recipe_id}` }}
              </RouterLink>
            </td>
            <td class="admin-actions"><button class="admin-del" @click="confirmDelete(r)">Törlés</button></td>
          </tr>
        </tbody>
      ...
      <template #default>
        <div v-if="editing" class="edit-form">
          <template v-if="editing.entity === 'users'">
            <BaseInput v-model="editing.form.username" label="Felhasználónév" :error="editErrors.username?.[0]" />
            <BaseInput v-model="editing.form.email" type="email" label="E-mail" :error="editErrors.email?.[0]" />
...
.admin-title { font-size: 1.625rem; font-weight: 800; color: var(--color-text); }
.admin-sub { font-size: 0.875rem; color: var(--color-muted); }
...
.admin-actions > * + * { margin-left: 0.35rem; }
...
.admin-edit:hover { background: var(--color-surface); }
.admin-del {
  color: var(--color-danger);
  border-color: color-mix(in srgb, var(--color-danger) 40%, transparent);
}
.admin-del:hover { background: color-mix(in srgb, var(--color-danger) 10%, transparent); }
.admin-del:disabled,
.admin-edit:disabled { opacity: 0.4; cursor: not-allowed; }

.edit-form { display: flex; flex-direction: column; gap: 0.75rem; }
.edit-textarea-label { display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.8rem; font-weight: 700; color: var(--color-text); }
.edit-textarea {
  border: 1.5px solid var(--color-stroke);
  border-radius: 0.5rem;
  padding: 0.5rem 0.75rem;
...
.edit-check { display: inline-flex; gap: 0.4rem; font-size: 0.85rem; font-weight: 600; align-items: center; }
.edit-check input[type="checkbox"] { accent-color: var(--color-accent); }
.edit-err { color: var(--color-danger); font-size: 0.8rem; }
.del-confirm { font-size: 0.9rem; color: var(--color-text); }
</style>
