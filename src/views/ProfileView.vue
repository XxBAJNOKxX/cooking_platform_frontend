<!-- Saját profil: receptek, eszközök, kedvencek + beállítások modal. -->
  ...
  <div class="profile-page">
    ...
    <div class="profile-card">
      ...
      <div class="profile-info">
        ...
        <p v-if="profile?.phone" class="profile-meta">
          ...
          >
            <path
              d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 11a19.79 19.79 0 01-3.07-8.67A2 2 0 012 .84h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"
            />
          </svg>
          {{ profile.phone }}
        </p>
    ...
    <section v-if="activeTab === 'recipes'" class="panel" role="tabpanel">
      ...
      <div v-else-if="recipes.length === 0" class="empty-state">
        ...
        <RouterLink :to="{ name: 'recipe-create' }" class="empty-cta"
          >Hozz létre egyet →</RouterLink
        >
      </div>
    ...
    <section v-else-if="activeTab === 'tools'" class="panel" role="tabpanel">
      ...
      <div v-else class="items-grid">
        <div v-for="(tool, i) in tools" :key="tool.id" class="item-wrap">
          ...
          <div class="item-actions">
            <RouterLink
              :to="{ name: 'tool-edit', params: { id: tool.id } }"
              class="action-btn action-edit"
              title="Szerkesztés"
              aria-label="Szerkesztés"
            >
    ...
    <section v-else-if="activeTab === 'favorites'" class="panel" role="tabpanel">
      ...
      <div
        ...
        class="items-grid"
        aria-busy="true"
        aria-label="Kedvencek betöltése"
      >
        <RecipeSkeletonCard v-for="i in 6" :key="i" :index="i" />
      </div>
    ...
    >
      <p class="del-text">
        Biztosan törölni szeretnéd a/az
        <strong>{{ deleteTarget?.name ?? deleteTarget?.title }}</strong>
        {{ deleteKind === 'tool' ? 'eszközt' : 'receptet' }}? Ez a művelet nem vonható vissza.
      </p>
