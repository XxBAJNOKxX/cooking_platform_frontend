...
<template>
  <BaseModal :model-value="modelValue" title="Profil szerkesztése" max-width="max-w-md"
    ...
    <div class="form-body">
      ...
      <div class="field">
        ...
        <input id="s-username" v-model="form.username" class="inp" :class="{ 'inp-err': errors.username }" type="text" autocomplete="username" />
        <p v-if="errors.username" class="field-err">{{ errors.username[0] }}</p>
      </div>

      <div class="field">
        <label class="lbl" for="s-phone">Telefonszám</label>
        <input id="s-phone" v-model="form.phone" class="inp" :class="{ 'inp-err': errors.phone }" type="tel" autocomplete="tel" placeholder="+36 30 123 4567" />
        <p v-if="errors.phone" class="field-err">{{ errors.phone[0] }}</p>
      </div>

      <div class="field">
        <label class="lbl" for="s-bio">Bio</label>
        <textarea id="s-bio" v-model="form.bio" class="inp inp-ta" :class="{ 'inp-err': errors.bio }" rows="3" placeholder="Mondd el ki vagy és mit főzöl szívesen…" />
...
.success-bar svg { width: 1rem; height: 1rem; }

.success-bar-enter-active { transition: opacity 200ms var(--ease-ui-out), transform 200ms var(--ease-ui-out); }
.success-bar-enter-from   { opacity: 0; transform: translateY(-4px) scale(0.97); }
...
.inp-err { border-color: var(--color-danger); }
...
.avatar-inputs { flex: 1; display: flex; flex-direction: column; gap: 0.375rem; }
...
.avatar-upload-btn {
  ...
  cursor: pointer;
  transition: background 150ms ease, border-color 150ms ease, color 150ms ease, transform 150ms var(--ease-ui-out);
  align-self: flex-start;
}
.avatar-upload-btn svg { width: 0.875rem; height: 0.875rem; flex-shrink: 0; }
.avatar-upload-btn:hover:not(:disabled) { border-color: var(--color-accent); color: var(--color-accent); background: color-mix(in srgb, var(--color-accent) 6%, transparent); }
.avatar-upload-btn:active:not(:disabled) { transform: scale(0.96); }
.avatar-upload-btn:disabled { opacity: 0.5; cursor: not-allowed; }

.hidden-file { display: none; }

@keyframes spin { to { transform: rotate(360deg); } }
.avatar-spin { animation: spin 0.8s linear infinite; }
</style>
