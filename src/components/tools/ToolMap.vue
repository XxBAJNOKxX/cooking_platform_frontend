...
function renderSecondary() {
  ...
  for (const m of props.markers) {
    const layer = m.radius_m > 0
      ? makeCircle(m.lat, m.lng, m.radius_m)
      : makeMarker(m.lat, m.lng)
...
watch(() => [props.lat, props.lng, props.radiusM, props.label], () => {
  renderPrimary()
...
<template>
  <div class="tmap-outer">
    ...
    <p class="tmap-note">
      A pontos cím bérlés egyeztetése után derül ki.
    </p>
  </div>
</template>

<style scoped>
.tmap-outer { display: flex; flex-direction: column; gap: 0.5rem; }
...
.tmap-pin:hover .tmap-pin-dot { transform: scale(1.12); }
