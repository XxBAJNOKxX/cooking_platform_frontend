...
<template>
  <div class="aac">
    ...
    <div class="aac-wrap">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="aac-icon" aria-hidden="true">
      ...
      <ul v-if="open && results.length" class="aac-list" role="listbox">
        ...
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="aac-pin" aria-hidden="true">
...
<style scoped>
.aac { display: flex; flex-direction: column; gap: 0.375rem; width: 100%; }

.aac-label {
  display: flex; justify-content: space-between; align-items: baseline;
  font-size: 0.875rem; font-weight: 500; color: var(--color-muted);
}
.aac-err { color: var(--color-danger); font-size: 0.75rem; font-weight: 600; }
...
.aac-input-err { border-color: var(--color-danger); }
...
@keyframes aacSpin { to { transform: translateY(-50%) rotate(360deg); } }
...
@starting-style {
  .aac-list { opacity: 0; transform: translateY(-4px); }
}
